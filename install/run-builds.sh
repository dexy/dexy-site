...
function run_script_in_ec2 {
  ...
  cat script-header > $temp_filename
  ...
  cat /sites/dexy-site/install/$1 >> $temp_filename
  echo "END" >> $temp_filename
  echo "bash script.sh &> out.txt" >> $temp_filename
  echo "cd .." >> $temp_filename
  cat /sites/dexy-site/install/close.sh >> $temp_filename

  ec2-run-instances $ami_id -k ec2-keypair \
